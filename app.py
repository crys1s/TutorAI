...
def process_audio_data():
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as temp_audio:
            ...
            temp_audio.flush()
            print("temp_audio.name = ", temp_audio.name) # On windows temp_audio.name returns the full file path, which is bad
            print("bucket_name = ", bucket_name)
            filename = os.path.basename(temp_audio.name)
            print("filename = ", filename)

            response = s3.upload_file(temp_audio.name, bucket_name, filename)
